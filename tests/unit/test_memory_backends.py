...
class TestInMemoryStorageBackend:
    ...
    async def test_get_nonexistent(self, backend):
        """Test getting a non-existent object raises ObjectNotFound."""
        with pytest.raises(ObjectNotFound):
            await backend.get("nonexistent")
        
        with pytest.raises(ObjectNotFound):
            await backend.get("nonexistent", "collection")
    ...
    async def test_remove_from_collection(self, backend, sample_objects):
        ...
        with pytest.raises(ObjectNotFound):
            await backend.get(obj_id, collection)
