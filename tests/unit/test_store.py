...
from activity_store.exceptions import InvalidLDObject, ObjectNotFound
...
class TestActivityStore:
    ...
    async def test_setup_teardown(self):
        ...
        mock_backend.setup.reset_mock()
        mock_cache.setup.reset_mock()
        await store.setup()
        mock_backend.setup.assert_not_called()
        mock_cache.setup.assert_not_called()
        ...
        # Teardown should not be called again
        mock_backend.teardown.reset_mock()
        mock_cache.teardown.reset_mock()
        await store.teardown()
        mock_backend.teardown.assert_not_called()
        mock_cache.teardown.assert_not_called()
    ...
    async def test_dereference_nonexistent(self, activity_store):
        """Test dereferencing a non-existent object raises ObjectNotFound."""
        with pytest.raises(ObjectNotFound):
            await activity_store.dereference("nonexistent")
    ...
    async def test_remove_from_collection(self, activity_store, sample_object):
        ...
        with pytest.raises(ObjectNotFound):
            await activity_store.backend.get(sample_object["id"], collection)
