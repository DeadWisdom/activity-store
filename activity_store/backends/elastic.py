...
from ..exceptions import ActivityStoreError, ObjectNotFound
...
DEFAULT_MAPPINGS = {
    "properties": {
        ...
        "summary": {"type": "text"},
        "tag": {"type": "keyword"},
        "published": {"type": "date", "format": "date_optional_time||strict_date_optional_time"},
        "updated": {"type": "date", "format": "date_optional_time||strict_date_optional_time"},
...
class ElasticsearchBackend(StorageBackend):
    ...
    async def setup(self) -> None:
        """Set up the Elasticsearch client and create required indices."""
        if not self._initialized:
            if self._client is None:
                try:
                    ...
                    logger.error(
                        f"Failed to connect to Elasticsearch at {self.es_url}",
                        metadata={"error": str(e)}
                    )
                    raise ActivityStoreError(f"Elasticsearch connection failed: {e}") from e
            ...
            logger.info(
                f"Connected to Elasticsearch at {self.es_url}",
    ...
    async def teardown(self) -> None:
        ...
        if self._initialized and self._client is not None:
            try:
                ...
                logger.error(
                    "Error during Elasticsearch teardown",
                    metadata={"error": str(e)}
                )
            finally:
                self._initialized = False
    ...
    async def add(
        ...
        try:
            ...
            if collection:
                ...
                logger.info(
                    f"Added object {object_id}",
                    metadata={"object_id": object_id}
                )
        except Exception as e:
            logger.error(
                f"Failed to add object {object_id}",
                metadata={"object_id": object_id, "collection": collection, "error": str(e)}
            )
    ...
    async def remove(self, id: str, collection: Optional[str] = None) -> None:
        ...
        try:
            ...
            # Not found is expected in some cases, so log but don't raise
            logger.warning(
                f"Object {id} not found for removal",
                metadata={"object_id": id, "collection": collection}
            )
            # Still raise ObjectNotFound to maintain consistency with interface
            raise ObjectNotFound(f"Object {id} not found")
        except Exception as e:
            logger.error(
                f"Failed to remove object {id}",
                metadata={"object_id": id, "collection": collection, "error": str(e)}
            )
            raise ActivityStoreError(f"Elasticsearch remove operation failed: {e}") from e
    ...
    async def get(self, id: str, collection: Optional[str] = None) -> Dict[str, Any]:
        ...
        Raises:
            ObjectNotFound: If the object doesn't exist in the specified location
        """
        if not self._initialized:
            await self.setup()
        ...
        try:
            ...
            logger.warning(
                f"Object {id} not found",
                metadata={"object_id": id, "collection": collection}
            )
            raise ObjectNotFound(f"Object {id} not found")
        except Exception as e:
            logger.error(
                f"Failed to get object {id}",
                metadata={"object_id": id, "collection": collection, "error": str(e)}
            )
            raise ActivityStoreError(f"Elasticsearch get operation failed: {e}") from e
    ...
    async def query(self, query: Query) -> Dict[str, Any]:
        ...
        if "collection" in query_dict and query_dict["collection"]:
            es_query["query"]["bool"]["must"].append({
                "term": {"_collection": query_dict["collection"]}
            })
        ...
        try:
            ...
            logger.error(
                "Query failed",
                metadata={"query": query_dict, "error": str(e)}
            )
            raise ActivityStoreError(f"Elasticsearch query operation failed: {e}") from e